#' @title Funkcja zapisuje parametry skalowania do bazy danych
#' @description
#' Funkcja zapisuje parametry skalowania do bazy danych
#' @param id_skali id skali. Gdy id skali nie jest NULL to parametry: nazwa_skali i id_testu nie są brane pod uwagę
...
#' @param numer_skalowania liczba całkowita określająca numer skalowania. Jeżeli wynosi NULL to wartość zostanie określona na podstawie bazy danych.
#' @param grupa nazwa grupy, dla której mają być wstawione parametry
#' @param zrodloDanychODBC string określający źródło danych. Wartość domyślna to 'EWD'.
#' @return Funkcja zwraca id skalowania.
#' @import RODBCext
#' @export
zapisz_parametry_skalowania = function(id_skali =NULL, nazwa_skali=NULL, id_testu=NULL,
                                       parametry, rEAP = NULL, numer_skalowania = NULL,
                                       grupa = "", zrodloDanychODBC="EWD") {
  stopifnot(is.numeric(numer_skalowania) | is.null(numer_skalowania))
  # proste sprawdzenie poprawności daty
  #   if(!grepl("^[[:digit:]]{4}-[[:digit:]]{2}-[[:digit:]]{2}", data))
  #   {
  #     stop("Data powinna być w formacie: RRRR-MM-DD. Paramer data wynosi:", data)
  #   }
  if( is.null(id_skali) & is.null(nazwa_skali) & is.null(id_testu) ){
    stop("Nazwa skali, id skali, id testu nie mogą mieć jednocześnie wartości null.")
  }
  ...
  if(grupa %in% unique(parametry$grupa)){
    parametry = parametry[parametry$grupa == grupa, ]
  } else{
    stop("Nie ma podanej grupy w kolumnie grupa parameru 'parametry'.")
  }
  ...
  tryCatch({
    ...
    if(is.null(id_skali)){
      # zapytanie dotyczące poszukiwanych skal
    ...
    if(is.null(numer_skalowania)){
      ...
      numerSkalowania = numer_skalowania
    }
  ...
  tryCatch({
    ...
    for(k in seq_along(liczbaParamKryteria)){
      ...
      if( liczbaParamKryteria[k]== 1 ){
        ...
        wstaw_do_skalowania_elementy(P, idSkali, kolejnoscTemp, numerSkalowania,
                                     ...
                                     parBy$'S.E.'[parByNumeryKryteriow %in% krytNum], grupa)
        
        wstaw_do_skalowania_elementy(P, idSkali, kolejnoscTemp, numerSkalowania,
                                     "trudność", "2PL",
                                     parTreshold$wartosc[parTresholdNumeryKryteriow %in% krytNum]/parBy$wartosc[parByNumeryKryteriow %in% krytNum],
                                     parTreshold$'S.E.'[parTresholdNumeryKryteriow %in% krytNum]/parBy$wartosc[parByNumeryKryteriow %in% krytNum], grupa)
        ...
        wstaw_do_skalowania_elementy(P, idSkali, kolejnoscTemp, numerSkalowania,
                                     "trudność", "GRM", srednia, NULL, grupa)
        ...
        for(m in indTres ){
          ...
          wstaw_do_skalowania_elementy(P, idSkali, kolejnoscTemp, numerSkalowania, nazwaPar, "GRM",
                                       ...
                                       parTreshold$'S.E.'[m]/dyskryminacja, grupa)
        }
      }
    }
    ...
    for(k in seq_along(liczbaParamPseudok)){
      ...
      if( liczbaParamPseudok[k]== 1 ){
        ...
        wstaw_do_skalowania_elementy(P, idSkali, kolejnoscTemp, numerSkalowania,
                                     ...
                                     parBy$'S.E.'[parByNumeryPseudo %in% pseudoNum], grupa)
        
        wstaw_do_skalowania_elementy(P, idSkali, kolejnoscTemp, numerSkalowania,
                                     "trudność", "2PL",
                                     parTreshold$wartosc[parTresholdNumeryPseudo %in% pseudoNum]/parBy$wartosc[parByNumeryPseudo %in% pseudoNum],
                                     parTreshold$'S.E.'[parTresholdNumeryPseudo %in% pseudoNum]/parBy$wartosc[parByNumeryPseudo %in% pseudoNum], grupa)
        ...
        wstaw_do_skalowania_elementy (P, idSkali, kolejnoscTemp,
                                      numerSkalowania, "dyskryminacja",
                                      "GRM", dyskryminacja, parBy$'S.E.'[parByNumeryPseudo %in% pseudoNum], grupa)
    ...
    for(ir in seq_along(nazwySpecjalne)){
      ...
      wstaw_do_skalowania_elementy(P, idSkali, NULL, numerSkalowania,
                                   ...
                                   parBy$'S.E.'[parBy$zmienna2 %in% kryt], uwagi = kryt, grupa)
      
      wstaw_do_skalowania_elementy(P, idSkali, NULL, numerSkalowania,
                                   "trudność", "2PL",
                                   parTreshold$wartosc[parTreshold$zmienna1 %in% kryt]/parBy$wartosc[parBy$zmienna2 %in% kryt],
                                   parTreshold$'S.E.'[parTreshold$zmienna1 %in% kryt]/parBy$wartosc[parBy$zmienna2 %in% kryt], uwagi = kryt, grupa)
    }
...
wstaw_do_skalowania_elementy = function(zrodloODBC, idSkali, kolejnosc, numerSkalowania,
  ...
  insSkalowania = paste0("INSERT INTO skalowania_elementy  (id_elementu, id_skali, ", ifelse(is.null(kolejnosc), "", "kolejnosc, "),
                         "skalowanie, parametr, ", ifelse(is.na(parametrGrupowy$grupowy), "", "grupowy, "),
                         " model, wartosc, uwagi", ifelse(is.null(odchylenie), "",", bs") ,
                         ifelse(is.null(grupa), "",", grupa"), ")
                         VALUES (nextval('skalowania_elementy_id_elementu_seq'), ?,", ifelse(is.null(kolejnosc), "", "?, "), " ?, ?, ?, ?,", ifelse(is.na(parametrGrupowy$grupowy),  "", " ?,"),
                         ifelse(is.null(uwagi), " ''", " ?"),
                         ifelse(is.null(odchylenie), "", ", ?" ),
                         ifelse(is.null(grupa), "",", ?"), ")"
                         )
  ...
  if(!is.null(grupa)){
    insertData = cbind(insertData, grupa)
  }
  #   print( insSkalowania)
  #   print(insertData)
