...
load('tests/testthat/dane/spr2011EWDskalowanie.RData')
skalowanie$id_skali = 11
skalowanie$oszacowania = skalowanie$oszacowania[1:10]
opisSkalowania = list(
  skalowanie     = 100,
  opis           = 'skalowanie testowe',
  estymacja      = 'MML (Mplus)',
  id_skali       = skalowanie$id_skali,
...
test_that('zapisz_skalowanie() tworzy skalowanie', {
  zapisz_skalowanie(skalowanie, opisSkalowania)
  wynik = sqlExecute(P, "SELECT * FROM skalowania WHERE id_skali = ? AND skalowanie = ?", list(opisSkalowania$id_skali, opisSkalowania$skalowanie), TRUE)
  
  sqlQuery(P, "DELETE FROM skalowania WHERE id_skali = ? AND skalowanie = ?", list(opisSkalowania$id_skali, opisSkalowania$skalowanie))
  
  expect_equal(wynik, opisSkalowania)
})
...
test_that('zapisz_skalowanie() dostosowuje skalę', {
  sqlExecute(P, "INSERT INTO skale_elementy VALUES (-1, 11, 1424, null, null)")
  
  zapisz_skalowanie(skalowanie, opisSkalowania)
  elementy = sqlExecute(P, "SELECT * FROM skale_elementy WHERE id_skali = ?", opisSkalowania$id_skali, TRUE)
  
  sqlExecute(P, "DELETE FROM skale_elementy WHERE id_skali = ? AND kolejnosc = -1", opisSkalowania$id_skali)
...
test_that('zapisz_skalowanie() zapisuje parametry', {
  zapisz_skalowanie(skalowanie, opisSkalowania)
  parametry = sqlExecute(P, "SELECT * FROM skalowania_elementy WHERE id_skali = ? AND skalowanie = ? ORDER BY parametr, kolejnosc", list(opisSkalowania$id_skali, opisSkalowania$skalowanie), TRUE)
  parametry$id_elementu = rep(NA, nrow(parametry))
  parametry$wartosc = round(parametry$wartosc, 3)
  parametry$bs = round(parametry$bs, 3)
  
  sqlExecute(P, "DELETE FROM skalowania_elementy WHERE id_skali = ? AND skalowanie = ?", list(opisSkalowania$id_skali, opisSkalowania$skalowanie))
  
  expect_equal(parametry, data.frame(
    id_skali    = rep(opisSkalowania$id_skali, 76),
